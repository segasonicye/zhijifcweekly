...
function getArticleTemplate(data, contentHTML, photos = [], logoPath = null, options = {}) {
  const { showScore = true, themeName } = options;
  const theme = getTheme(themeName || 'claude');
  ...
  const logoSection = `
    <div style="text-align: center; margin: 0 0 20px 0;">
      <img src="${logoSrc}" alt="知己FC" style="width: 72px; height: 72px; display: block; margin: 0 auto; border-radius: 50%; border: 3px solid ${accent}; box-shadow: 0 4px 16px rgba(0,0,0,0.1); object-fit: cover;" />
    </div>
  `;

  // ── 比赛信息框 ──
  const infoBox = `
    <section style="padding: 30px 25px; margin: 0 0 28px 0; border-radius: 12px; text-align: center; background: ${accent}; color: #fff; box-shadow: 0 8px 24px rgba(0,0,0,0.12);">
      <h1 style="font-size: 28px; margin: 0 0 20px; font-weight: 800; color: #fff; line-height: 1.4; letter-spacing: 0.5px;">${data.title || '知己FC 战报'}</h1>

      <div style="display: flex; justify-content: center; flex-wrap: wrap; gap: 10px; margin-bottom: 20px;">
        ${data.date ? `<span style="background: rgba(255,255,255,0.22); padding: 8px 18px; border-radius: 20px; font-size: 13px; color: #fff; font-weight: 500;">📅 ${data.date}</span>` : ''}
        ${data.opponent ? `<span style="background: rgba(255,255,255,0.22); padding: 8px 18px; border-radius: 20px; font-size: 13px; color: #fff; font-weight: 500;">⚽ ${data.opponent}</span>` : ''}
        ${data.location ? `<span style="background: rgba(255,255,255,0.22); padding: 8px 18px; border-radius: 20px; font-size: 13px; color: #fff; font-weight: 500;">📍 ${data.location}</span>` : ''}
      </div>

      ${data.score && showScore ? `
        <div style="background: rgba(255,255,255,0.18); padding: 16px 28px; border-radius: 10px; display: inline-block;">
          <span style="font-size: 36px; font-weight: 900; color: #fff; letter-spacing: 2px;">${data.score}</span>
        </div>
      ` : ''}
    </section>
  `;
  ...
  if (data.mvp) {
    mvpSection = `
      <section style="padding: 28px 25px; margin: 0 0 28px 0; border-radius: 12px; text-align: center; background: #fff; border: 2px solid ${accent}; box-shadow: 0 4px 16px rgba(0,0,0,0.06);">
        <p style="margin: 0 0 10px; font-size: 12px; letter-spacing: 3px; text-transform: uppercase; font-weight: 700; color: ${accent};">本场最佳 · MVP</p>
        <div style="font-size: 28px; font-weight: 800; color: ${accent};">${data.mvp}</div>
        <div style="margin-top: 10px; font-size: 14px; color: #888;">🌟 战斗英雄 🌟</div>
      </section>
    `;
  }
  ...
  if (data.attendance && data.attendance.length > 0) {
    attendanceSection = `
      <section style="padding: 24px; margin: 0 0 28px 0; border-radius: 10px; background: #fff; border-left: 4px solid ${accent}; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px;">
          <span style="font-size: 18px;">🛡️</span>
          <span style="font-size: 13px; letter-spacing: 2px; font-weight: 700; color: ${accent}; text-transform: uppercase;">出勤名单 · ${data.attendance.length}人</span>
        </div>
        <div style="line-height: 2.2; font-size: 15px;">${data.attendance.join(' · ')}</div>
      </section>
    `;
  }
    ...
    photosSection = `
      <section style="margin: 40px 0;">
        <div style="text-align: center; margin-bottom: 24px;">
          <span style="font-size: 13px; letter-spacing: 3px; text-transform: uppercase; font-weight: 700; color: ${accent};">📸 精彩瞬间</span>
        </div>
        ${photosHTML}
      </section>
    `;
  ...
  // ── 页脚 ──
  const footer = `
    <section style="padding: 30px 25px; margin: 50px 0 20px 0; border-radius: 12px; text-align: center; background: #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
  ...
  const article = `
    <div style="${theme.container} max-width: 680px;">
      ...
      <section style="${theme.p} text-align: justify; background-color: ${theme.container.match(/background-color:\s*([^;]+)/i)?.[1] || 'transparent'}; padding: 20px; border-radius: 8px; margin: 0 -10px;">
        ${applyThemeToHTML(contentHTML, theme)}
      </section>
