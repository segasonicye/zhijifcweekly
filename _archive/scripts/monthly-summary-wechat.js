...
<body>
  <section style="max-width: 650px; margin: 0 auto; background: #fff;">
      ...
      <section style="margin-bottom: 30px;">
        ...
        <table style="width: 100%; border: 1px solid #e8e8e8;" cellpadding="0" cellspacing="0">
          <tr style="background: #2c3e50; color: #fff;">
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left;">排名</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left;">球员</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left; width: 70px;">进球</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left; width: 80px;">效率</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left; width: 60px;">场次</th>
          </tr>
    ...
    return `<tr style="${bg}">
              <td style="padding: 14px 10px; font-size: 16px; border-bottom: 1px solid #e8e8e8;">${medal}</td>
              <td style="padding: 14px 10px; font-size: 16px; border-bottom: 1px solid #e8e8e8;">${item[0]}</td>
              <td style="padding: 14px 10px; font-size: 16px; border-bottom: 1px solid #e8e8e8; font-weight: bold; color: #e74c3c;">${item[1]}</td>
              <td style="padding: 14px 10px; font-size: 14px; border-bottom: 1px solid #e8e8e8; color: #7f8c8d;">${avg}</td>
              <td style="padding: 14px 10px; font-size: 14px; border-bottom: 1px solid #e8e8e8; color: #7f8c8d;">${playerAttendance}</td>
            </tr>`;
  }).join('')}
        </table>
      </section>
      ...
      <section style="margin-bottom: 30px;">
        ...
        <table style="width: 100%; border: 1px solid #e8e8e8;" cellpadding="0" cellspacing="0">
          <tr style="background: #2c3e50; color: #fff;">
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left;">排名</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left;">球员</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left; width: 70px;">次数</th>
            <th style="padding: 12px 10px; font-size: 15px; font-weight: 600; text-align: left; width: 80px;">出勤率</th>
          </tr>
          ${sortedAttendance.slice(0, 15).map((item, index) => {
    const rank = index + 1;
    const medal = rank === 1 ? '🥇' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : rank.toString().padStart(2, '0');
    const rate = ((item[1] / files.length) * 100).toFixed(0);
    const bg = rank <= 3 ? 'background: #fff3cd;' : rank % 2 === 0 ? 'background: #f8f9fa;' : 'background: #fff;';
    return `<tr style="${bg}">
              <td style="padding: 14px 10px; font-size: 16px; border-bottom: 1px solid #e8e8e8;">${medal}</td>
              <td style="padding: 14px 10px; font-size: 16px; border-bottom: 1px solid #e8e8e8;">${item[0]}</td>
              <td style="padding: 14px 10px; font-size: 16px; border-bottom: 1px solid #e8e8e8; font-weight: bold; color: #2ecc71;">${item[1]}</td>
              <td style="padding: 14px 10px; font-size: 14px; border-bottom: 1px solid #e8e8e8; color: #7f8c8d;">${rate}%</td>
            </tr>`;
  }).join('')}
        </table>
      </section>
...
// 运行逻辑：支持命令行参数
// 用法: node monthly-summary-wechat.js [year] [month]
// 示例: node monthly-summary-wechat.js 2026 2

const args = process.argv.slice(2);
const year = args[0] ? parseInt(args[0]) : new Date().getFullYear();
// 如果没传月份，默认统计上个月（如果是月初）或当前月？
// 这里逻辑是：如果没传，就默认统计当前月份
const month = args[1] ? parseInt(args[1]) : new Date().getMonth() + 1;

console.log(`🚀 开始生成月度总结: ${year}年${month}月...`);
const outputFile = generateWechatMonthlySummary(year, month);

// 打开预览
if (outputFile && !process.env.NO_OPEN) {
